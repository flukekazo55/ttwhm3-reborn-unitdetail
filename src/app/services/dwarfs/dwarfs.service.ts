...
export class DwarfsService {
  private readonly dataUrl = `${environment.apiUrl}/assets/data/dwarfs.json`;
  ...
  private toModel(response: GuideApiResponse<FactionGuideApiData>): FactionGuide {
    if (!response.data) {
      throw new Error(response.error ?? 'Invalid DWARFS guide response');
    }
