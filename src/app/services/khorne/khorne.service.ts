...
export class KhorneService {
  private readonly dataUrl = `${environment.apiUrl}/assets/data/khorne.json`;
  ...
  private toModel(response: GuideApiResponse<FactionGuideApiData>): FactionGuide {
    if (!response.data) {
      throw new Error(response.error ?? 'Invalid KHORNE guide response');
    }
